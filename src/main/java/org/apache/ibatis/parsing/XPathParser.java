 ...
 * 其实就是把xml文件解析为一个dom文档
 */
public class XPathParser {

    private Document document; //把xml解析为的dom文档树结构
    private boolean validation; //是否需要验证xml文件格式的正确性
    private EntityResolver entityResolver; //如何解析xml文件格式的声明
    private Properties variables; //properties属性 用户自己定义的配置文件，不是从配置文件解析来的
    private XPath xpath; //根据路径，解析xml文件
    ...
    public String evalString(Object root, String expression) {
        ...
        //2.再调用PropertyParser去解析,有时候配置文件里面引用其他地方的属性 出现${username}这种形式的值 需要进行替换
    ...
    private Object evaluate(String expression, Object root, QName returnType) {
        try {
            //最终合流到这儿，直接调用XPath.evaluate 通过解析的xml路径进行解析
    ...
    private void commonConstructor(boolean validation, Properties variables, EntityResolver entityResolver) {
        ...
        this.variables = variables;
        //共通构造函数，除了把参数都设置到实例变量里面去以外，还初始化了XPath解析器
