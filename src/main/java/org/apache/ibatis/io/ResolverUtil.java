...
public class ResolverUtil<T> {
   ...
   */
    //对包解析后 返回一个这个解析的类 类里面有个参数可以获取哪些符合条件的类
  public ResolverUtil<T> findImplementations(Class<?> parent, String... packageNames) {
    if (packageNames == null) {
      return this;
    }
    //循环遍历 每个包下的类 是否都是parent的子类
   ...
   * @param packageNames one or more package names to scan (including subpackages) for classes
   */
  //返回一个包下 哪些继承了 annotation注解
  public ResolverUtil<T> findAnnotated(Class<? extends Annotation> annotation, String... packageNames) {
    if (packageNames == null) {
      return this;
    }
  //通过写一个接口 然后有两种实现  只需调用相应的借口就行  方法只需处理一次
  ...
  public ResolverUtil<T> find(Test test, String packageName) {
    ...
    try {
        //通过VFS来深入jar包里面 列出所有文件的名字
      List<String> children = VFS.getInstance().list(path);
      //查找一个路径下所有的文件 【com/learn/mapper/a.class,com/learn/mapper/b.class,com/learn/demo/c.class】
   ...
   *  把配置文件中的.换成/  因为：packageName一般都是 com.example.demo
  ...
  protected void addIfMatching(Test test, String fqn) {
    try {
      //com/learn/mapper/a.class得到类的路径 需要利用类加载起加载进来 转化为类加载起的路径 --> com.learn.mapper.a
      String externalName = fqn.substring(0, fqn.indexOf('.')).replace('/', '.');
      ClassLoader loader = getClassLoader();
      log.debug("Checking to see if class " + externalName + " matches criteria [" + test + "]");
       //如果是.class文件 则通过类加载器开始加载这个类
