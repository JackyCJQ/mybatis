...
public class MapperAnnotationBuilder {
    ...
    //不管是注解配置还是xml配置最终都是通过 MapperBuilderAssistant来构建
    private MapperBuilderAssistant assistant;
    private Class<?> type; //对应的mapper接口

    public MapperAnnotationBuilder(Configuration configuration, Class<?> type) {
        //把com.example.mapper 转为 com/example/mapper.java (best guess)
    ...
    public void parse() {
        //interface com.example.mapper.UserMapper 这种格式
    ...
    /**
     * 这种通过加载类的方式，如果mapper接口中的方法不是基于注解的，就需要找到对应的xml配置文件，
     * 默认是同路径，同名字的xml文件
     */
    ...
    private void parseCacheRef() {
        ...
        if (cacheDomainRef != null) {
            //cacheDomainRef.value()是一个类 需要得到全路径 作为依据
    ...
    private Class<?> getParameterType(Method method) {
        ...
        for (int i = 0; i < parameterTypes.length; i++) {
            //如果参数不是RowBounds，ResultHandler这两个参数 其余的都是用户定义的
